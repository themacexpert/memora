...
class QdrantDB(BaseVectorDB):
    ...
    def __init__(self, 
        ...
        self.async_client.set_model(self.vector_embedding_model, cache_dir=embed_models_cache_dir)
        self.async_client.set_sparse_model(self.sparse_vector_embedding_model, cache_dir=embed_models_cache_dir)
    ...
    def _dense_embed_queries(self, queries: List[str]) -> List[List[float]]:
        ...
        return list(self.async_client.embedding_models[self.vector_embedding_model].embed(queries))
    
    def _sparse_embed_queries(self, queries: List[str]) -> List[List[float]]:
        """Embed queries using the sparse vector embedding model."""
        return list(self.async_client.sparse_embedding_models[self.sparse_vector_embedding_model].embed(queries))
