...
class Memora:
    ...
    def __init__(
            ...
            memory_search_model: BaseBackendLLM = GroqBackendLLM(api_key=os.getenv("GROQ_API_KEY"), model="llama-3.1-8b-instant"),
            extraction_model: BaseBackendLLM = GroqBackendLLM(api_key=os.getenv("GROQ_API_KEY"), model="llama-3.3-70b-specdec", max_tokens=8000, max_retries=0),
    ...
    async def generate_memory_search_queries(self,
        ...
        memory_search_queries = [arg.strip() for arg in arguments_passed] if arguments_passed else []
    ...
    async def filter_retrieved_memories_with_model(self,
        ...
        self.logger.debug(f"Number of retrieved memories to filter: {len(retrieved_memories)}")

        current_day_of_week = current_datetime.strftime('%A')
        self.logger.debug(f"Current day of week: {current_day_of_week}, datetime: {current_datetime.isoformat()}")
        ...
        if not selected_memories_ids: # The LLM misbehaved not extracting any memory_ids or << NONE >>.
            self.logger.warning("No memory IDs were extracted from the model response, due to LLM misbehavior.")
            return None
        
        # The LLM is undeterministic and can select the same memory_ids multiple times.
        filtered_ids = set([str(selection).strip() for selection in selected_memories_ids if str(selection).strip().lower() != "none"])
        self.logger.info(f"Memory filtering complete. Selected {len(filtered_ids)} unique memories")
        self.logger.debug(f"Selected memory IDs: {filtered_ids}")
        
        return filtered_ids

    async def search_memories_as_one(self, org_id: str, user_id: str, search_queries: List[str], filter_out_memory_ids_set: Set[str] = set(), agent_id: Optional[str] = None, search_across_agents: bool = True) -> List[Dict[str, str]]:
        """
        ...
        self.logger.debug(f"Agent context - agent_id: {agent_id}, memories_across_agents: {search_across_agents}")
        ...
        org_user_mem_ids = [
            { 'memory_id': memory['memory_id'], 'user_id': memory['user_id'], 'org_id': memory['org_id'] }
            for memory in sorted_memories
            if memory['memory_id'] not in filter_out_memory_ids_set
        ]

        if not org_user_mem_ids:
            self.logger.info("No memories retrieved or left after filtering out `filter_out_memory_ids_set`")
            return []
        
        return await self.graph.fetch_user_memories_resolved(org_user_mem_ids)
    
    async def search_memories_as_batch(self, org_id: str, search_queries: List[str], filter_out_memory_ids_set: Set[str] = set(), user_id: Optional[str] = None, agent_id: Optional[str] = None, memory_search_scope: MemorySearchScope = MemorySearchScope.USER, search_across_agents: bool = True) -> List[List[Dict[str, str]]]:
        """
        ...
        self.logger.debug(f"Search context - user_id: {user_id}, agent_id: {agent_id}, scope: {memory_search_scope}")
        ...
        return await self.graph.fetch_user_memories_resolved_batch(batch_org_user_mem_ids)
    ...
    async def save_or_update_interaction_and_memories(
        ...
        self.logger.debug(f"Interaction context - agent_id: {agent_id}, extract_agent_memories: {extract_agent_memories}")
        ...
        for retry in range(max_retries + 1):
            try:
                ...
                if interaction_id:
                    self.logger.debug(f"Fetching previously extracted memories for interaction {interaction_id}")
                    ...
                    self.logger.debug(f"Found {len(previously_extracted_memories)} previously extracted memories")
                ...
                self.logger.debug(f"Extracted {len(candidate_memories)} candidate memories")
                ...
                if interaction_id:
                    return await self.graph.update_interaction_and_memories(
                            ...
                            updated_memories_and_interaction=MemoriesAndInteraction(
                                ...
                                contrary_memories=[ContraryMemoryToStore(memory=memory_tuple[0], source_msg_block_pos=memory_tuple[1], existing_contrary_memory_id=memory_tuple[2]) for memory_tuple in new_contrary_memories]
                                ),
                            vector_db_add_memories_fn=self.vector_db.add_memories
                        )
                else:
                ...
                if retry == max_retries:
                    self.logger.error(f"Failed to save/update interaction after {max_retries} retries", exc_info=True)
                    raise
                else:
                    self.logger.warning(f"Attempt {retry + 1} failed, retrying...", exc_info=True)
                    continue
    ...
    async def get_memories_for_message(
        ...
        Returns:
           List[Dict[str, str]] | None: Memories to be recalled for the message, or None if no relevant memories found.
        """

        self.logger.info(f"Getting memories for message from user {user_id} in org {org_id}")
        self.logger.debug(f"Message context - agent_id: {agent_id}, context messages: {len(preceding_msg_for_context)}")
        self.logger.debug(f"Model-based filtering enabled: {enable_final_model_based_memory_filter}")
